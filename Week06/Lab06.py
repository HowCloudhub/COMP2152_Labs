...
def do_ping():
    ...
    print("  Packets:     " + str(ping_data["transmitted"]) +
          " sent, " + str(ping_data["received"]) + " received")
    print("  Packet Loss: " + ping_data["loss"])
    print("  Avg Latency: " + str(ping_data["avg_ms"]) + " ms")

    log_to_csv(LOG_FILE, "ping", host,
               ping_data["avg_ms"], ping_data["status"])
...
def do_nslookup():
    ...
    log_to_csv(LOG_FILE, "nslookup", domain,
               dns_data["ip"], dns_data["status"])
    print("Result logged.")
...
def do_network_info():
    ...
    try:
        ...
        log_to_csv(LOG_FILE, "ipconfig", "all",
...
def do_arp_table():
    ...
    try:
        ...
        log_to_csv(LOG_FILE, "arp", "local", str(
            len(devices)) + " devices", "Captured")
        print("\nResult logged.")
    except Exception as e:
        print("  Error: " + str(e))
